...
оставляет лучший результат. Если оба запроса не прошли, используется более
медленный `api.ipify.org`. После смены адреса `ipify` запускается обязательно,
даже при успешном Apple URL Test, чтобы подтвердить реальный выход через новую
конфигурацию.
...
- первая подтверждённая ошибка получает статус `Suspected` и проверяется через
  30 секунд, вторая — ещё раз через 2 минуты;
- три неудачных раунда переводят ноду в `Needs replacement`; затем она не
  нагружает канал и проверяется вручную, после изменения или раз в 6 часов;
- `Unstable` перепроверяется через 5 минут;
- стабильные ноды распределяются по окну 45–60 минут, поэтому новой большой
  пачки не возникает;
...
- новый endpoint сразу добавляется и проверяется, а пропавший сначала получает
  `Missing` и удаляется только после 30 успешных опросов без появления;
...
записям подписки. Перепроверить можно как одну привязку, так и всю ноду.
...
Индикатор рядом с `Auto` показывает состояние маршрута iPhone и мобильный IP:
...
| `POST /api/v1/proxies/{stableID}/recheck` | Поставить одну ноду на ручную перепроверку |
...
| `/api/v1/network` | Состояние маршрута iPhone и текущий мобильный IP |
