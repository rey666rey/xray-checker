...
در صورت فعال بودن، پس از راه‌اندازی یک بررسی کامل اجرا می‌شود، نتایج در حافظه نگه داشته می‌شوند و بررسی‌های کامل زمان‌بندی‌شده غیرفعال می‌شوند. با راه‌اندازی مجدد فرایند، یک بررسی کامل جدید آغاز می‌شود. این گزینه به‌روزرسانی اشتراک را غیرفعال نمی‌کند؛ اگر مجموعه پروکسی‌ها نباید تغییر کند، `SUBSCRIPTION_UPDATE` را نیز غیرفعال کنید.
