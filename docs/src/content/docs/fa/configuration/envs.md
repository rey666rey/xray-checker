...
تعداد پاسخ‌های مستقل اشتراک که در هر چرخه به‌روزرسانی ادغام می‌شوند. وقتی یک هاست بین چند نود فیزیکی می‌چرخد، این مقدار را افزایش دهید. هر endpoint جدید فقط یک بار بررسی می‌شود و نمونه‌های اضافی بررسی نودهای شناخته‌شده را تکرار نمی‌کنند. Compose محلی از `4` استفاده می‌کند.
...
در صورت فعال بودن، پس از راه‌اندازی یک بررسی کامل اجرا می‌شود و بررسی‌های انبوه زمان‌بندی‌شده غیرفعال می‌شوند. پایشگر تعمیر همچنان گره‌های مشکوک، ناپایدار یا تغییرکرده را به‌صورت هدفمند بررسی می‌کند. راه‌اندازی مجدد یک بررسی کامل تازه آغاز می‌کند.
...
مسیر فایل JSON وضعیت که توسط یک پایشگر خارجی شبکه به‌روزرسانی می‌شود. در صورت تنظیم، نبودن، قدیمی بودن یا قطع بودن وضعیت، بررسی‌های جدید پروکسی را متوقف می‌کند. درخواست‌هایی که هنگام قطع تأییدشده شبکه مختل شوند، پس از بازیابی دوباره اجرا می‌شوند و offline ذخیره نمی‌شوند.
...
حداکثر سن قابل قبول فایل وضعیت بر حسب ثانیه. اگر پایشگر بیش از این مدت فایل را به‌روزرسانی نکند، بررسی پروکسی تا دریافت وضعیت connected تازه متوقف می‌شود.
...
مسیر یک فایل JSON پایدار برای نتایج پروکسی. مجموعه تکمیل‌شده پس از راه‌اندازی دوباره فرایند یا ماشین مجازی بازیابی می‌شود و با `PROXY_INITIAL_CHECK_ONLY=true`، اگر برای همه پروکسی‌های فعلی نتیجه ذخیره شده باشد، پیمایش کامل تکرار نمی‌شود.
...
مسیر تاریخچه پایدار گره‌های منطقی. این فایل تغییر آدرس، شمارنده‌های تأیید، وضعیت تعمیر، IP خروجی و رویدادهای اخیر را بین اجراهای تازه نگه می‌دارد.
...
مسیر تاریخچه پایدار تشخیص دستی گره‌ها. تا ده گزارش برای هر گره فیزیکی نگه‌داری می‌شود؛ شامل رابط و IP مبدأ، نتیجه TCP/TLS مستقیم و تلاش‌های واقعی تونل Xray. این گزارش‌ها وضعیت عادی online/offline را تغییر نمی‌دهند و پس از تغییر پیکربندی به‌صورت قدیمی علامت‌گذاری می‌شوند.
