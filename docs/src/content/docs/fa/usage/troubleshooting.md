...
1. URL اشتراک را تأیید کنید
2. بررسی کنید URL هنوز فعال است
...
- فضای خالی یا خط جدید اضافی

**راه‌حل‌ها:**

1. تأیید کنید رشته Base64 تمیز و بدون فضای خالی است
2. اگر استاندارد ناموفق بود دیکد URL-safe Base64 را امتحان کنید
3. بررسی کنید محتوا نیاز به دیکد چندباره دارد
...
2. قوانین فایروال را تأیید کنید
...
- احراز هویت فعال است
- اعتبارنامه‌های نادرست
- پورت اشتباه

**راه‌حل‌ها:**

1. بررسی کنید احراز هویت فعال است:
   ```bash
   METRICS_PROTECTED=false
   ```
2. اعتبارنامه‌ها را تأیید کنید:
   ```bash
   METRICS_USERNAME=user
   METRICS_PASSWORD=pass
   ```
3. پورت صحیح را تأیید کنید:
   ```bash
   METRICS_PORT=2112
   ```
...
1. پورت متریک را تغییر دهید:
   ...
   METRICS_PORT=2113
   ```
2. فرآیندهای در حال اجرا را بررسی کنید:
   ```bash
   lsof -i :2112
   ```
...
```
این لاگ‌های دقیق برنامه را در حالی که خروجی Xray را قابل مدیریت نگه می‌دارد ارائه می‌دهد.
:::
