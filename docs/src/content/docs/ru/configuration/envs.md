...
Если включено, после запуска выполняется одна полная проверка прокси, результаты сохраняются в памяти, а плановые полные перепроверки отключаются. Перезапуск процесса запускает новую полную проверку. Эта настройка не отключает обновление подписки; если набор прокси не должен меняться, также отключите `SUBSCRIPTION_UPDATE`.
