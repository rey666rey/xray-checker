...
Количество независимых ответов подписки, объединяемых в каждом цикле. Увеличьте, если один хост случайно выбирает одну из нескольких физических нод. Каждый новый endpoint проверяется один раз; дополнительные снимки не повторяют проверки уже известных адресов. Локальный Compose использует `4`.
...
Если включено, после запуска выполняется одна полная проверка, а плановые массовые перепроверки отключаются. Точечный монитор ремонта продолжает проверять подозрительные, нестабильные и изменившиеся ноды. Перезапуск процесса запускает новую полную проверку.
...
- Обязательно: Нет
- По умолчанию: `ip`
- Значения: `urltest`, `ip`, `status`, `download`

Метод, используемый для проверки функциональности прокси:

- `urltest`: Выполняет несколько маленьких GET через прокси, сохраняет лучший успешный пинг и обращается к IP endpoint только при провале всех основных попыток
...
При `PROXY_CHECK_METHOD=urltest` этот URL используется только как независимый fallback после провала всех основных URL-тестов.

### PROXY_URL_TEST_URL

- CLI: `--proxy-url-test-url`
- Обязательно: Нет
- По умолчанию: `http://captive.apple.com/hotspot-detect.html`

Маленькая страница, запрашиваемая через каждый прокси методом `urltest`.
...
Подстрока, обязательная в успешном ответе. Пустое значение разрешает любой ответ 2xx.
...
Количество независимых основных запросов. Сохраняется минимальный успешный TTFB; частичный успех отмечает ноду как unstable.
...
Таймаут одного запроса в секундах для медленной перепроверки ошибок первой проходки.
...
Параллельность медленной перепроверки. `0` использует половину `PROXY_CHECK_CONCURRENCY`.
...
Путь к JSON-файлу состояния, который обновляет внешний монитор сети. Если настройка указана, отсутствие, устаревание или отключённое состояние файла ставит новые проверки прокси на паузу. Запросы, прерванные подтверждённым отключением сети, повторяются после восстановления и не сохраняются как offline.
...
Максимальный допустимый возраст файла состояния в секундах. Если монитор не обновляет файл дольше этого времени, проверки прокси приостанавливаются до появления свежего статуса connected.
...
Путь к постоянному JSON-снимку результатов прокси. Завершённый набор восстанавливается после перезапуска процесса или VM, а при `PROXY_INITIAL_CHECK_ONLY=true` полная проходка не повторяется, если сохранён результат каждого текущего прокси.
...
Путь к постоянной истории проверяемых привязок хостов к нодам. В отличие от удаляемого снимка полной проходки, этот файл сохраняет смены адресов, статусы ремонта, выходные IP и последние события между новыми запусками. Пока контейнер работает, опросы подписки накапливают пул endpoint-ов каждого хоста; endpoint удаляется только после 30 успешных опросов без появления.
...
Путь к постоянной истории ручной глубокой диагностики. Checker хранит до десяти отчётов для каждой физической ноды: интерфейс и исходный IP проверки, результаты прямого TCP/TLS и попытки через настоящие Xray-привязки. Эти отчёты не меняют обычный online/offline-статус и помечаются устаревшими после изменения конфигурации.
